...
class API_interface():
    ...
    def insert_dataset(self, project_name: str, experiment_name: str, dataset_in: d.Dataset):
        ...
        dataset_in.author = [d.Author(name=self.username,
                                      permission="write").dict()]  # assign admin permissions for new dataset the
        # user adds
    ...
    def return_fullexperiment(self, project_name: str, experiment_name: str):
        ...
        response = requests.get(
            self.path + project_name + "/" + experiment_name + "/names")  # request the names of the datasets connected to experiment
    ...
    def return_fullproject(self, project_name: str):
        ...
        return d.Project(name=proj_dict.get("name"), creator=proj_dict.get("creator"), groups=experiments,
                         meta=proj_dict.get("meta"), author=proj_dict.get("author"))
    ...
    def check_experiment_exists(self, project_name: str, experiment_name: str):
        ...
        if names is not None and experiment_name in names:
            return True
        else:
            return False
